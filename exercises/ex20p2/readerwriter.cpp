...
void taskWriter(int tid, int nbIterations, AbstractReaderWriter *resource,
...
void taskReader(int tid, int nbIterations, AbstractReaderWriter *resource,
                ReaderWriterChecker *checker) {
  std::random_device
      rd; // Will be used to obtain a seed for the random number engine
  std::mt19937 gen(rd()); // Standard mersenne_twister_engine seeded with rd()
...
protected:
  void testReaderWriterPrioWriter(int nbThreadsA, int nbThreadsB,
                                  int nbIterations) {
    ASSERT_DURATION_LE(15, {
      ...
      for (int t = 0; t < nbThreadsB; t++) {
        ...
        threadsB[t] = std::make_unique<std::thread>(
            taskReader, t, nbIterations, protocol.get(), checker.get());
      }
      for (int t = 0; t < nbThreadsA; t++) {
        logger() << "In main: creating thread A " << t << std::endl;
        threadsA[t] = std::make_unique<std::thread>(
            taskWriter, t, nbIterations, protocol.get(), checker.get());
      }
