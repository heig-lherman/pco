...
void taskProducer(AbstractBuffer<int> *buffer, Checker *checker,
                  Scenario *scenario, int nbProduce) {
  std::random_device
...
void taskConsumer(AbstractBuffer<int> *buffer, Checker *checker,
                  int nbConsume) {
  std::random_device
      rd; // Will be used to obtain a seed for the random number engine
  std::mt19937 gen(rd()); // Standard mersenne_twister_engine seeded with rd()
  ...
  for (int i = 0; i < nbConsume; i++) {
    ...
    logger()
        << "                                  Tache consommateur a récupéré: "
        << item << endl;
    PcoThread::usleep(dis(gen));
  }
}

template <typename T> class ProdConsTest : public ::testing::Test {
protected:
  void test() {
    ASSERT_DURATION_LE(20, {
      ...
      auto checker =
          std::make_unique<Checker>(NB_THREADS_PRODUCER, NB_THREADS_CONSUMER,
                                    NB_PRODUCTIONS_PER_PRODUCER);
      ...
      PcoManager::getInstance()->setMaxSleepDuration(
          1000, PcoManager::EventType::WaitConditionWait);
      for (int i = 0; i < NB_THREADS_PRODUCER; i++) {
        producers[i] = std::make_unique<PcoThread>(
            taskProducer, buffer.get(), checker.get(), scenario.get(),
            NB_PRODUCTIONS_PER_PRODUCER);
      }
      for (int i = 0; i < NB_THREADS_CONSUMER; i++) {
        consumers[i] = std::make_unique<PcoThread>(
            taskConsumer, buffer.get(), checker.get(),
            NB_CONSUMPTIONS_PER_CONSUMER);
      }
      for (int i = 0; i < NB_THREADS_PRODUCER; i++) {
        producers[i]->join();
      }
...
using MyTypes = ::testing::Types<Buffer2ConsoHoare<int>>;
TYPED_TEST_CASE(ProdConsTest, MyTypes);

TYPED_TEST(ProdConsTest, Standard) { this->test(); }
